...
- [Kailash Mansarovar Yatra](https://www.zeotourism.com/kailash-mansarovar): Complete Kailash Yatra packages for Indian pilgrims and NRIs
- [Kailash Mansarovar Yatra Guide 2025](https://www.zeotourism.com/kailash-mansarovar-yatra-guide): Complete pilgrimage guide — routes, permits, cost, best time, NRI info
...
- [All Tours](https://www.zeotourism.com/tours): Complete tour catalog
`;

    // Add tours dynamically
    tours.forEach(tour => {
      llmsTxt += `- [${tour.title}](https://www.zeotourism.com/tours/${tour.slug}): ${tour.duration || ''}${tour.difficulty ? ' | ' + tour.difficulty : ''} — ${tour.description ? tour.description.substring(0, 150) + '...' : ''}\n`;
    });

    llmsTxt += `\n## Destinations (Dynamic)\n- [All Destinations](https://www.zeotourism.com/destinations): Discover Nepal, Tibet, Bhutan destinations\n`;
    destinations.forEach(dest => {
      const slug = dest.slug || dest.name.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and');
      llmsTxt += `- [${dest.name}](https://www.zeotourism.com/destinations/${slug}): ${dest.description ? dest.description.substring(0, 150) + '...' : ''}\n`;
    });

    llmsTxt += `\n## Activities (Dynamic)\n- [All Activities](https://www.zeotourism.com/activities): Trekking, mountaineering, jungle safaris and cultural tours\n`;
    activities.forEach(act => {
      const slug = act.slug || act.name.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and');
      llmsTxt += `- [${act.name}](https://www.zeotourism.com/activities/${slug}): ${act.description ? act.description.substring(0, 150) + '...' : ''}\n`;
    });

    llmsTxt += `\n## Blog Posts (Dynamic)\n- [Blog](https://www.zeotourism.com/blog): Kailash Yatra guides, Nepal travel tips\n`;
    posts.slice(0, 10).forEach(post => {
      llmsTxt += `- [${post.title}](https://www.zeotourism.com/blog/${post.slug}): ${post.summary || post.excerpt || ''}\n`;
    });

    llmsTxt += `
## About & Contact
- [About Zeo Tourism](https://www.zeotourism.com/about): Founded 2000, Thamel Kathmandu
- [Contact](https://www.zeotourism.com/contact): Free consultation for Indian pilgrims and NRI travelers
...
* **URL**: https://www.zeotourism.com/tours/${tour.slug}
...
Link: https://www.zeotourism.com/destinations/${dest.slug || dest.name.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and')}
...
Link: https://www.zeotourism.com/activities/${act.slug || act.name.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and')}
...
Link: https://www.zeotourism.com/blog/${post.slug}
