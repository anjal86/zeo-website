          ...
          <table className="w-full table-fixed divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {tours.length === 0 ? (
                ...
                tours.map((tour) => (
                  <tr key={tour.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/admin/tours/${tour.slug}`)}>
                    ...
                    <td className="px-3 py-4 text-right space-x-2" onClick={e => e.stopPropagation()}>
                      <div className="flex justify-end gap-2" onClick={e => e.stopPropagation()}>
                        <button onClick={() => router.push(`/admin/tours/${tour.slug}`)} className="text-blue-600 hover:text-blue-900" title="Edit">
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t">
                ...
                <div className="flex gap-3" onClick={e => e.stopPropagation()}>
                  <button onClick={() => router.push(`/admin/tours/${tour.slug}`)} className="text-blue-600"><Edit className="w-4 h-4" /></button>
                  <button
                    onClick={() => handleExportSingleTour(tour)}
                    disabled={exportingId === tour.id}
