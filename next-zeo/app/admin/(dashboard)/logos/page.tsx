...
const LogosManager: React.FC = () => {
    ...
    const LogoField = ({ type, label }: { type: string; label: string }) => {
                ...
                {logo?.image_url ? (
                    <div className="relative inline-block">
                        ...
                        <div className="absolute top-1 right-1 flex gap-1">
                            ...
                            <button type="button" onClick={() => handleDelete(type)} className="bg-red-500/80 text-white p-1 rounded"><Trash2 className="w-3 h-3" /></button>
                        </div>
                    </div>
                ) : <p className="text-sm text-gray-500 mb-3">No logo uploaded</p>}
                <div className="mt-4">
                    <input type="file" accept="image/*" className="hidden" id={`logo-${type}`} onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(type, f); }} />
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <LogoField type="header" label="Header Logo" />
                <LogoField type="footer" label="Footer Logo" />
                <LogoField type="favicon" label="Favicon" />
            </div>
        </div>
    );
};
