      ...
      slides.forEach((slide) => {
        if (slide.video && !preloadedVideos.has(slide.video)) {
          ...
          video.addEventListener('loadedmetadata', () => {
            if (slide.video) {
              setPreloadedVideos(prev => new Set([...prev, slide.video]));
            }
          });

          video.addEventListener('error', () => {
            if (slide.video) {
              setVideoErrors(prev => new Set([...prev, slide.video]));
            }
          });
    ...
    return (
      <section id="home" className="relative min-h-screen w-full overflow-hidden flex items-center justify-center pt-24 bg-black">
        <div className="text-center text-white px-6">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold mb-4">Kailash Mansarovar Yatra & Nepal Tours</h1>
          <p className="text-lg sm:text-xl">Plan sacred journeys and Himalayan adventures with a Kathmandu-based team.</p>
        </div>
      </section>
    );
  }
    ...
    >
            ...
            {activeVideo && !isMobile && !videoErrors.has(activeVideo) ? (
              <video
                ...
                onLoadedMetadata={(e) => {
                  ...
                  if (activeSlide.video_start_time) {
                    video.currentTime = activeSlide.video_start_time;
                  }
                }}
                onError={() => {
                  if (activeVideo) {
                    setVideoErrors(prev => new Set([...prev, activeVideo]));
                  }
                }}
              >
      ...
      {!isMobile && (
        <div className="absolute inset-0 pointer-events-none">
          ...
          <motion.div animate={{ x: [0, -100, 0] }} transition={{ duration: 25, repeat: Infinity, ease: 'linear' }} className="absolute top-40 right-0 w-96 h-32 bg-white/5 rounded-full blur-3xl" style={{ y: cloud2Y }} />
      ...
      >
        <div className="w-full px-6 sm:px-12 lg:px-16">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.div variants={textVariants} className="text-primary text-xs font-bold tracking-[0.25em] uppercase mb-5" style={{ y: locationY }}>
                {activeSlide.location || 'Nepal'}
              </motion.div>

              <motion.h1 variants={textVariants} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-white mb-5 leading-tight" style={{ y: titleY }}>
                {activeSlide.title}
              </motion.h1>

              <motion.p variants={textVariants} className="text-sm sm:text-base text-white/60 mb-8 max-w-lg mx-auto leading-relaxed" style={{ y: subtitleY }}>
                {activeSlide.subtitle || ''}
              </motion.p>

              {activeSlide.show_button && (
                <motion.div variants={textVariants} className="flex justify-center" style={{ y: buttonY }}>
                  <motion.a
                    href={activeSlide.button_url || '/tours'}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className={`px-8 py-3.5 text-white font-bold text-xs tracking-wider uppercase transition-all duration-300 inline-flex items-center gap-2 ${activeSlide.button_style === 'secondary'
                      ? 'bg-secondary hover:bg-secondary-dark'
                      : activeSlide.button_style === 'outline'
                        ? 'border border-white/30 bg-transparent hover:bg-white hover:text-gray-900'
                        : 'bg-primary hover:bg-primary-dark'
                      }`}
                  >
                    {activeSlide.button_text || 'Explore Adventures'}
      ...
      {slides.length > 1 && (
        <>
          <button onClick={goToPrevSlide} className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-30 w-10 h-10 flex items-center justify-center text-white/50 hover:text-white transition-all duration-300" aria-label="Previous slide">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button onClick={goToNextSlide} className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-30 w-10 h-10 flex items-center justify-center text-white/50 hover:text-white transition-all duration-300" aria-label="Next slide">
            <ChevronRight className="w-5 h-5" />
          </button>
        </>
      )}

      <motion.div className="absolute bottom-8 md:bottom-6 w-full z-20" style={{ y: indicatorsY }}>
          ...
          <div className="flex items-center gap-2">
            {slides.map((_, index) => (
              <button
                ...
                className={`transition-all duration-300 ${index === currentSlide ? 'w-8 h-1.5 bg-primary' : 'w-2 h-1.5 bg-white/30 hover:bg-white/50'}`}
              />
            ))}
          </div>
        </div>
      ...
      <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20" style={{ y: scrollIndicatorY }}>
      ...
      {activeVideo && !isMobile && (
        <motion.div className="absolute bottom-4 left-4 z-30" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.3 }}>
          <motion.button onClick={toggleMute} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="glass p-2 sm:p-3 rounded-full text-white hover:bg-white/20 transition-all duration-300 backdrop-blur-md border border-white/20" aria-label={isMuted ? 'Unmute video' : 'Mute video'}>
