  ...
  <section className="py-14 md:py-16 bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
    <div className="container-xl">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="h-[430px] animate-pulse bg-gray-100" />
        <div className="h-[430px] animate-pulse bg-gray-100" />
      </div>
    </div>
  </section>
...
const EmptyState = ({ reset }: { reset: () => void }) => (
  <div className="border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm">
    ...
    <button onClick={reset} className="mt-5 inline-flex items-center border border-gray-200 px-5 py-2 text-sm font-bold text-gray-800 transition-colors hover:border-primary hover:text-primary">
      Clear filters
    </button>
  </div>
);
      ...
      <section className="py-16 bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
        <div className="container-xl">
          <div className="border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm">
            ...
            <Link href="/contact" className="mt-6 inline-flex items-center bg-primary px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-primary-dark">
  ...
  return (
    <section id="blog" className="py-14 md:py-16 bg-gradient-to-b from-white to-gray-50 relative overflow-hidden border-t border-gray-100">
      ...
      <div className="container-xl relative z-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
          ...
          >
            ...
            <div className="relative z-10 flex min-h-[430px] flex-col justify-between p-6 md:p-8 text-white">
              <div className="flex flex-wrap items-center gap-3">
                <span className="bg-secondary px-4 py-2 text-xs font-bold uppercase tracking-wider text-white">Start here</span>
                <span className="bg-white/15 backdrop-blur-sm px-3 py-2 text-xs font-bold uppercase tracking-wider text-white">{postCategory(featuredPost)}</span>
              </div>
              ...
              <div>
                ...
                <p className="mt-4 max-w-2xl text-sm md:text-base leading-7 text-white/80 line-clamp-3">{featuredPost.excerpt || "Read this guide before planning your next trip."}</p>
                <div className="mt-6 inline-flex items-center bg-white px-5 py-3 text-sm font-bold text-gray-950 transition-transform group-hover:translate-x-1">
          ...
          >
            <div className="flex items-center justify-between border-b border-gray-100 p-5 md:p-6">
              ...
              <span className="bg-gray-100 px-3 py-1 text-xs font-bold text-gray-700">{articleCount} posts</span>
            </div>
            ...
            <div className="border-t border-gray-100 p-5 md:p-6">
              <Link href="/contact" className="flex items-center justify-between border border-gray-200 bg-gray-50 px-5 py-4 text-sm font-bold text-gray-950 transition-colors hover:border-primary hover:bg-white hover:text-primary">
        ...
        <div className="mt-12 border border-gray-200 bg-white p-5 shadow-sm md:p-6">
          <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            ...
            <div className="relative w-full lg:w-[360px]">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search articles..." className="w-full border border-gray-200 bg-gray-50 py-3 pl-11 pr-11 text-sm outline-none transition-colors focus:border-primary focus:bg-white" />
          ...
          <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
            {categories.map((item) => (
              <button key={item} onClick={() => setCategory(item)} className={`shrink-0 px-4 py-2 text-xs font-bold transition-colors ${category === item ? "bg-primary text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                {item}
              </button>
            ))}
          </div>
          ...
          {filteredPosts.length === 0 ? <EmptyState reset={resetFilters} /> : (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredPosts.map((post) => (
                <Link key={post.id} href={postHref(post)} className="group overflow-hidden border border-gray-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-gray-900/10">
                  <div className="relative h-52 overflow-hidden bg-gray-100">
                    ...
                    <span className="absolute left-4 top-4 bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-gray-900 backdrop-blur">{postCategory(post)}</span>
                  </div>
                  <div className="p-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mt-12 border border-primary/15 bg-primary text-white p-6 md:p-8">
          <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-wider"><Mail className="h-3.5 w-3.5" /> Newsletter</div>
            ...
            {newsletterStatus === "success" ? (
              ...
              <form onSubmit={handleNewsletterSubmit} className="flex w-full flex-col gap-3 sm:w-[420px] sm:flex-row">
                <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Enter your email" required disabled={newsletterStatus === "loading"} className="min-w-0 flex-1 border border-white/25 bg-white/15 px-4 py-3 text-sm text-white placeholder-white/70 outline-none transition-all duration-300 focus:bg-white/20 disabled:opacity-50" />
                <motion.button type="submit" whileHover={{ scale: newsletterStatus === "loading" ? 1 : 1.03 }} whileTap={{ scale: 0.98 }} disabled={newsletterStatus === "loading"} className="bg-white px-5 py-3 text-sm font-bold text-primary transition-all duration-300 hover:bg-gray-100 disabled:opacity-60 disabled:cursor-not-allowed">
