...
// API Protection Middleware: Prevent direct scraping by checking referer
app.use('/api', (req, res, next) => {
  const referer = req.get('referer') || '';
  const allowedHost = 'zeotourism.com';
  const devHost = 'localhost';

  // Skip protection for documentation or specific internal tools if needed
  if (req.path.startsWith('/public') || req.path.startsWith('/uploads')) {
    return next();
  }

  // Basic check: Ensure request comes from our domain or localhost (dev)
  if (!referer.includes(allowedHost) && !referer.includes(devHost) && process.env.NODE_ENV === 'production') {
    console.warn(`Blocked suspicious API request from referer: ${referer}`);
